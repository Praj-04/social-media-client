...
export const getFeedData = createAsyncThunk(
  ...
  async () => {  
    try {
      const response = await axiosClient.get("/user/getFeedData");  //sending the userid is body
...
export const followAndUnfollowUser = createAsyncThunk('user/followAndUnfollow', async(body) =>{
  try {
   const response = await axiosClient.post("/user/follow",body);  //sending the userid is body
